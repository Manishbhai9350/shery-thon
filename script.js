...
function main() {
  ...
  themers.forEach((themer , i) => {
    ...
    gsap.to(themer,{
      scrollTrigger:{
        ...
        onUpdate(){
         if (document.body.getAttribute('theme') !== theme  && currentThemerIndex == i) {
            document.body.setAttribute('theme',theme)
         }
        }
      }
  ...
  function changingTextAnimation() {
    ...
    function addSpans() {
      var item = data[i]
      clutter += `<span class='changing-span'>${item == ' ' ? '&nbsp;' : item}</span>`
...
float_text.innerHTML = creteSpans(float_text || 'sheryians experience' , 'SHERIANS')
...
lenis.on('scroll' , e => {
  ...
  const rect = page4.getBoundingClientRect()
  const top = rect.top 
  if ((top > 0 ||  (rect.top + rect.height ) < rect.height ) && isParrallexShowen) {
    hideGallery()
  }
  ...
  gsap.to(page5Content , {
    scrollTrigger : {
      ...
      onUpdate(e){
        ...
        page5Content.style.transform = `${initialTransfrom} translateY(${offsetY - 50}%)`
      }
    }
  })
}
